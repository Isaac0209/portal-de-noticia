...
<?php
	if(isset($_POST['env']) && $_POST['env'] == "post"){
		if($_POST['titulo'] && $_POST['post'] && $_POST['resumo']){
			...
			$query = $con->prepare("INSERT INTO posts (id_postador, titulo, data, postagem, resumo, categoria, imagem) VALUES (?, ?, ?, ?, ?, ?, ?)");
			$query->bind_param("sssssss", $idUser, $titulo, $data, $post, $resumo, $cat, $uploadfileN);
			$query->execute();
